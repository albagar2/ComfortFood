...
<div class="min-h-screen flex flex-col">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
            <div class="flex items-center gap-8">
                ...
                <nav class="hidden md:flex gap-6 text-sm font-medium text-zinc-600 dark:text-zinc-400">
                    <a href="#" class="hover:text-zinc-900 dark:hover:text-white flex items-center gap-1 text-zinc-950">
                ...
                @if (Route::has('login'))
                    @auth
                        <a href="{{ url('/dashboard') }}"
                           class="text-sm font-medium text-zinc-600 hover:text-zinc-900 dark:text-zinc-400 dark:hover:text-white">Dashboard</a>
                    @else
                        <a href="{{ route('login') }}" class="text-sm font-medium text-blue-600 hover:text-blue-500">Iniciar
                            sesión</a>

                        @if (Route::has('login'))
                            <a href="{{ route('login') }}"
                               class="px-4 py-2 bg-zinc-600 text-white rounded-md text-sm font-medium hover:bg-zinc-700">¡Comienza
                                ya!</a>
                        @endif
                    @endauth
                @endif
    ...
    <main class="flex-grow">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
            <div class="text-center md:text-left">
                ...
                <p class="text-lg text-zinc-600 dark:text-zinc-400 mb-8 max-w-lg mx-auto md:mx-0">
    ...
    <!-- Footer -->
    <footer class="w-full py-4 border-t border-white/10 bg-zinc-900 backdrop-blur-sm">
        <div class="container mx-auto px-6">
            <div class="flex flex-col items-center justify-between space-y-4 md:flex-row md:space-y-0">
                ...
                <div class="text-[11px] uppercase tracking-[0.2em] text-gray-200 font-light text-center">
