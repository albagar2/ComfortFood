<div class="p-6">
    ...
    <!-- Empty State -->
    @if($menus->isEmpty())
        <div class="text-center py-12 bg-white dark:bg-zinc-900 rounded-xl border border-dashed border-zinc-200 dark:border-zinc-800">
            ...
            <flux:button href="{{ route('dashboard') }}" wire:navigate variant="primary" class="mt-6">Explorar Menús</flux:button>
        </div>
    @else
            ...
            @foreach($menus as $menu)
                <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-4 shadow-sm flex flex-col gap-4 relative group hover:border-zinc-300 dark:hover:border-zinc-700 transition-colors">
                    <div class="flex justify-between items-start">
                         ...
                         <button wire:click="toggleFavorite({{ $menu->id_menu }})" class="text-red-500 hover:text-red-700 transition-colors">
                    ...
                    <div class="flex items-center justify-between pt-4 border-t border-zinc-100 dark:border-zinc-800">
                         <flux:button size="sm" variant="primary" class="w-full">Ver detalles</flux:button>
                    </div>
                </div>
            @endforeach
        </div>
