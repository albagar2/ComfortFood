<div wire:poll.15s wire:key="completed-orders-badge" class="relative inline-flex group/badge">
    ...
    @if($total > 0)
        <!-- Single Grey Badge -->
        <span
            class="absolute -top-7.5 -right-23 flex h-4 w-4 items-center justify-center rounded-full bg-emerald-500 text-[10px] font-bold text-white ring-1 ring-navy-light border border-white/10 z-20 cursor-help">
            {{ $total > 9 ? '9+' : $total }}
        </span>
