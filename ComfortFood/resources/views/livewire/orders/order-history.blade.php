    ...
    <div
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm text-left text-zinc-600 dark:text-zinc-400">
                <thead
                    class="text-[11px] text-zinc-700 uppercase bg-zinc-50/50 dark:bg-zinc-800/50 border-b border-zinc-100 dark:border-zinc-800 font-bold tracking-wider">
                    <tr>
                        ...
                        <tr class="group hover:bg-zinc-50 dark:hover:bg-zinc-800/50 transition-colors">
                            <td class="px-6 py-5">
                                <div class="flex items-center gap-2">
                                    <span
                                        class="font-bold text-blue-600 dark:text-blue-400">#{{ $pedido->id_pedido }}</span>

                                    @if(!$isRestaurant)
                                        @if($pedido->estado->nombre_estado === 'Cancelado')
                                            <div class="relative group/tooltip">
                                                <div
                                                    class="size-2 rounded-full bg-rose-500 shadow-sm shadow-rose-500/50 animate-pulse">
                                                </div>
                                                <div
                                                    class="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-2 py-1 bg-zinc-900 text-white text-[10px] font-bold rounded-lg opacity-0 group-hover/tooltip:opacity-100 transition-opacity whitespace-nowrap z-30 pointer-events-none border border-white/10 shadow-xl">
                                                    {{ __('Pedido Cancelado') }}
                                                </div>
                                            </div>
                            ...
                            <td class="px-6 py-5">
                                <div class="flex flex-col">
                                    <span
                                        class="font-semibold text-zinc-950 dark:text-zinc-100">{{ $pedido->created_at->format('d/m/Y') }}</span>
                            ...
                            <td class="px-6 py-5">
                                <div class="flex items-center gap-3">
                                    ...
                                    @endphp
                                    <flux:avatar size="sm" :src="$displayUser->profile_photo_url"
                                        :name="$displayUser->nombre_completo" />
                                    <span
                                        class="font-semibold text-zinc-950 dark:text-zinc-100">{{ $displayUser->nombre_completo }}</span>
                                </div>
                            </td>
                            ...
                            <td class="px-6 py-5">
                                ...
                                @endphp
                                <span
                                    class="px-3 py-1 rounded-full text-[11px] font-bold uppercase tracking-tight {{ $statusColor }}">
                                ...
                                @if($pedido->estado->nombre_estado === 'Entregado' && !$isRestaurant)
                                    ...
                                    @else
                                        <flux:button variant="filled" size="sm"
                                            wire:click="openReviewModal({{ $pedido->id_pedido }})"
                                            class="!bg-pastel-orange !text-white !rounded-xl text-[10px] font-bold py-1">
                                            Valorar
                                        </flux:button>
                                    @endif
                                @endif
    ...
    <flux:modal wire:model="showReviewModal" class="md:w-96">
        <div class="space-y-6">
            ...
            <div class="space-y-4">
                    ...
                    <div class="flex gap-1">
                        @for($i = 1; $i <= 5; $i++)
                            <button wire:click="$set('rating', {{ $i }})"
                                ...
                                <flux:icon.star variant="{{ $rating >= $i ? 'solid' : 'outline' }}"
                ...
                <flux:textarea wire:model="comment" label="Comentario (opcional)"
                    placeholder="Cuéntanos qué te pareció el pedido..." rows="4" />
            </div>

            <div class="flex gap-2 justify-end">
                <flux:button variant="ghost" @click="$wire.showReviewModal = false">Cancelar</flux:button>
                <flux:button variant="filled" wire:click="saveReview" class="!bg-pastel-orange !text-white">Enviar
                    Valoración</flux:button>
            </div>
        </div>
    </flux:modal>
</div>